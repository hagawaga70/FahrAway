...
CREATE TABLE fahrt (
	...
	bis				date,
	kl_ku			varchar (15),
	f_unterkunft	integer REFERENCES unterkunft (u_id)
);
...
CREATE TABLE aktivitaet (
	...
	dauer	integer,
	akpreis	numeric (5, 2),
	anbietr integer REFERENCES anbieter (an_id)	
);
...
/*	bezieht ist obsolet ---> DROP TABLE!, da Fremdschluessel in Aktivitaet auf Anbieter verweist [anbietr REFERENCES anbieter (a_id)]
CREATE TABLE bietetan (
	...
	bezieht ist obsolet ---> DROP TABLE!, da Fremdschluessel in Fahrt auf Unterkunft verweist [F_Unterkunft REFERENCES unterkunft (u_id)]
CREATE TABLE bezieht (
	...
	istin ist obsolet ---> DROP TABLE!, da Fremdschluessel in unterkunft auf ort verweist [U_Ort REFERENCES Ort (o_id)]
CREATE TABLE istin (
	u_id	integer,
	o_id	integer,
...
/*
Hinzufuegen Fremdschluesselbezuege:

[nachtraeglich z.B.: ALTER TABLE istin ADD FOREIGN KEY (u_id) REFERENCES unterkunft (u_id) ON UPDATE CASCADE ON DELETE CASCADE]
/*
